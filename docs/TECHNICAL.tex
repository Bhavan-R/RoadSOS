...
|   |- /triage    (Claude Haiku 4.5 + rule-based fallback)      |
...
AI & anthropic 0.34.2 (Claude Haiku 4.5) \\
...
\file{backend/services/ai\_triage.py} reorders a contact list to match the user's situation. The model is \texttt{claude-haiku-4-5-20251001}. Two boolean situational flags are accepted: \code{injured} and \code{blocking\_traffic}.
...
Triage is a separate endpoint (\code{POST /triage}) called only after the user answers the situational questions. The map and contact list render with no AI dependency; if Anthropic is entirely unreachable, the list is still useful.
...
services:
  - type: web
    ...
    envVars:
      - key: ANTHROPIC_API_KEY    # required
        sync: false
...
|  |- requirements.txt           # runtime deps (FastAPI, httpx, anthropic, ...)
...
|  |  |- ai_triage.py            # Claude Haiku 4.5 + rule fallback
...
AI model & \code{claude-haiku-4-5-20251001} \\
